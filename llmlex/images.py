...
def generate_base64_image_ha(state_data: np.ndarray,
                              ...
                              show: bool = False) -> str:
    """
    Generates a base64 encoded PNG image for hybrid automata data.
    
    This function creates a visualization similar to plot_fun but returns a base64
    encoded image suitable for LLM analysis via llmlex.single_call.
    
    Args:
        state_data (np.ndarray): 2D array with shape (num_states, num_steps).
        input_data (np.ndarray): Input data (empty, 1D, or 2D array).
        dt (float): Time step size.
        system_name (str, optional): Name of the system. Defaults to "System".
        sample_index (int, optional): Sample index for title. Defaults to None.
    ...
    Returns:
        str: The base64 encoded string of the PNG image.
    """
    logger.debug(f"Generating base64 image for HA data: {state_data.shape}")
    ...
    try:
        ...
        # Save figure to buffer
        logger.debug("Saving figure to buffer for base64 encoding")
        buffer = io.BytesIO()
        fig.savefig(buffer, format='png', dpi=100)
        buffer_size = buffer.tell()
        logger.debug(f"Buffer size: {buffer_size} bytes")
